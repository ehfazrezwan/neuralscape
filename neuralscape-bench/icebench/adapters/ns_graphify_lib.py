"""
NS-Graphify-Lib adapter: Graphify-lib engine via NS REST code-graph routed endpoints.

Indexes and queries a corpus through the NS REST API using `knowledge_system=code-graphify-lib`.
...
Capabilities: symbol_lookup, neighbors_1hop, path_le4, blast_radius
N/A: nl_locate (graphify-lib has no dense-embedding locate → raises)
...
class NSGraphifyLibAdapter:
    """Adapter for NS using Graphify-lib engine via routed REST endpoints."""
    ...
    def __init__(
        ...
        with `system=code-graphify-lib`, polls for completion, then queries through
        the routed code-graph endpoints with `knowledge_system=code-graphify-lib`.
    ...
    def _read_version(self) -> str:
        ...
        try:
            ...
            if r.status_code == 200:
                return f"ns-graphify-lib@{r.json().get('service', 'neuralscape')}"
        except Exception:
            pass
        return "ns-graphify-lib@unknown"

    def capabilities(self) -> set[str]:
        """
        NS-Graphify-Lib supports 4 ops via routed REST endpoints.

        - symbol_lookup: routed query
        - neighbors_1hop: routed neighbors
        - path_le4: routed path (variable-length up to 4)
        - blast_radius: routed impact analysis

        NOT supported:
        - nl_locate: graphify-lib has no dense-embedding locate → raises
        """
    ...
    def index_cold(self, corpus: Corpus) -> IndexResult:
        """
        Index via async POST /v1/code-graph/index with system=code-graphify-lib.
    ...
    def index_incremental(self, corpus: Corpus, touched: list[str]) -> IndexResult:
        """Graphify-lib through NS doesn't support incremental => re-run cold."""
    ...
    def store_size_bytes(self, corpus: Corpus) -> int:
        ...
        access to isolate per-code_space storage. Return 0 with this docstring
        note (honest N/A).
        """
        return 0

    def export_snapshot(self, corpus: Corpus) -> SnapshotResult | None:
        """Snapshot export not yet implemented for ns-graphify-lib (honest N/A)."""
        return None

    def import_snapshot(self, corpus: Corpus, blob_path: str) -> SnapshotResult | None:
        """Snapshot import not yet implemented for ns-graphify-lib (honest N/A)."""
        return None

    def query(self, op: str, payload: dict) -> QueryResult:
        """Execute a query via NS REST code-graph tools with knowledge_system=code-graphify-lib."""
        if op not in self.capabilities():
            raise UnsupportedOp(f"Operation {op} not supported by ns-graphify-lib")
            ...
            if op == "symbol_lookup":
                resp = self.client.get(
                    ...
                    params={
                        ...
                        "knowledge_system": "code-graphify-lib",
                        "graph_id": code_space,
                    },
                )
